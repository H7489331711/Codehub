...
// ── Person Card ──────────────────────────────────────────────
function PersonCard({ name, role, image, bio, facts }: {
  name: string; role: string; image: string; bio: string; facts?: string[];
}) {
  return (
    <div className="ch-person-card">
      <div className="ch-person-left">
        <img src={image} alt={name} className="ch-person-img" />
        <div className="ch-person-name">{name}</div>
        <div className="ch-person-role">{role}</div>
      </div>
      <div className="ch-person-right">
        <p className="ch-person-bio">{bio}</p>
        {facts && facts.length > 0 && (
          <div className="ch-person-facts">
            {facts.map((f, i) => (
              <div key={i} className="ch-person-fact">
                <span className="ch-person-fact-dot">▸</span>
                <span>{f}</span>
              </div>
            ))}
          </div>
...
function ImageBlock({ src, alt, caption }: { src: string; alt: string; caption?: string }) {
  return (
    <div className="ch-image-block">
      <img src={src} alt={alt} className="ch-image" />
      {caption && <p className="ch-image-caption">{caption}</p>}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* ── Person Card ── */
        .ch-person-card { display:flex; gap:20px; border:1px solid #e0e7ff; border-radius:14px; padding:20px; margin:16px 0; background:linear-gradient(135deg,#f0f4ff,#fff); }
        .ch-person-left { flex-shrink:0; text-align:center; width:120px; }
        .ch-person-img { width:100px; height:100px; border-radius:50%; object-fit:cover; border:3px solid #3b82f6; box-shadow:0 4px 12px rgba(59,130,246,.25); display:block; margin:0 auto 8px; }
        .ch-person-name { font-size:14px; font-weight:700; color:#1d4ed8; }
        .ch-person-role { font-size:11px; color:#6b7280; margin-top:2px; background:#eff6ff; padding:2px 8px; border-radius:10px; display:inline-block; }
        .ch-person-right { flex:1; }
        .ch-person-bio { font-size:13.5px; color:#374151; line-height:1.75; margin:0 0 12px; }
        .ch-person-facts { display:flex; flex-direction:column; gap:5px; }
        .ch-person-fact { display:flex; gap:8px; align-items:baseline; font-size:13px; color:#4b5563; }
        .ch-person-fact-dot { color:#3b82f6; font-size:14px; flex-shrink:0; }

        /* ── Image Block ── */
        .ch-image-block { margin:16px 0; text-align:center; }
        .ch-image { max-width:100%; border-radius:10px; border:1px solid #e5e7eb; box-shadow:0 2px 8px rgba(0,0,0,.08); }
        .ch-image-caption { font-size:12px; color:#9ca3af; margin-top:6px; font-style:italic; }

        @media(max-width:600px){
          .ch-person-card { flex-direction:column; } .ch-person-left { width:100%; display:flex; align-items:center; gap:12px; text-align:left; }
          ...
          <div className="ch-content-card">
            ...
            if (block.type === "person") return (
              <PersonCard
                ...
                image={block.image}
                bio={block.bio}
                facts={"facts" in block ? block.facts : undefined}
              />
            );
