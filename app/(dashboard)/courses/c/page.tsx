...
import { cChapters, levels } from "@/app/component/data/cData";
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6 flex items-center gap-5">
          <div className="w-20 h-20 bg-gray-100 rounded-2xl flex items-center justify-center shrink-0">
            <img src="/c.png" alt="C Language" className="w-14 h-14 object-contain" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-800">C Programming Language Complete Course</h1>
            <p className="text-gray-500 text-sm mt-1">
              {cChapters.length} chapters • 2 levels • Beginner to Intermediate
            </p>
          </div>
        </div>
        ...
        {levels.map(({ level, name, icon, color, chapters }) => {
          const levelChapters = cChapters.filter((c) => c.level === level);
