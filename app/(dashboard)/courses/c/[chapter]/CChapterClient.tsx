...
import { Chapter, QuizQuestion } from "@/app/component/data/cData";
...
function CodeBlock({ code, label }: { code: string; label?: string }) {
  ...
  return (
    <div className="my-4 rounded-xl overflow-hidden border border-gray-200">
      ...
      <pre className="bg-gray-900 text-yellow-300 text-sm p-4 overflow-x-auto leading-relaxed">
        <code>{code}</code>
      </pre>
    </div>
  );
...
function ExampleBlock({ code, output, label }: { code: string; output?: string; label?: string }) {
  return (
    <div className="my-4 rounded-xl overflow-hidden border border-yellow-200">
      <div className="bg-yellow-600 px-4 py-2">
        <span className="text-xs text-white font-medium">▶ {label || "Example"}</span>
      </div>
      <pre className="bg-gray-900 text-yellow-300 text-sm p-4 overflow-x-auto">{code}</pre>
      {output && (
        <>
  ...
  return (
    <div className="mt-8 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="bg-gray-800 px-6 py-4">
        ...
        <p className="text-gray-400 text-sm">{quiz.length} questions • Test your knowledge</p>
      </div>
      ...
      {!submitted ? (
        <div className="p-6">
          ...
          <div className="w-full h-1.5 bg-gray-100 rounded-full mb-6">
            <div className="h-1.5 bg-gray-700 rounded-full transition-all"
          ...
          <div className="flex items-center justify-between">
            ...
            {current < quiz.length - 1 ? (
              <button onClick={() => setCurrent((p) => p + 1)}
                className="px-4 py-2 text-sm bg-gray-800 hover:bg-gray-700 text-white rounded-xl">
                Next →
              </button>
            ) : (
            ...
            {quiz.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)}
                className={`w-7 h-7 rounded-full text-xs font-medium transition-all ${
                  i === current ? "bg-gray-800 text-white"
        ...
        <div className="p-6">
          ...
          <button onClick={() => { setAnswers({}); setSubmitted(false); setCurrent(0); }}
            className="w-full mt-4 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl font-medium text-sm">
            Retake Quiz
          </button>
        </div>
      )}
...
export default function CChapterClient({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-200 p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <Link href="/courses/c" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-800 mb-4 transition-colors">
          ← Back to C Course
        </Link>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
          {chapter.content.map((block, i) => {
            ...
            if (block.type === "tip") return (
              <div key={i} className="flex gap-3 bg-yellow-50 border border-yellow-200 rounded-xl p-4 my-4">
                <span className="text-yellow-600 text-lg shrink-0">💡</span>
                <p className="text-yellow-800 text-sm leading-relaxed">{block.content}</p>
              </div>
            );
        ...
        <div className="flex items-center justify-between mt-6 gap-4">
          {prevChapter ? (
            <Link href={`/courses/c/${prevChapter.id}`}
              className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-3 hover:border-gray-400 hover:bg-gray-50 transition-colors group">
              <p className="text-xs text-gray-400 mb-0.5">← Previous</p>
              <p className="text-sm font-medium text-gray-700 group-hover:text-gray-900 truncate">{prevChapter.title}</p>
            </Link>
          ) : <div className="flex-1" />}

          {nextChapter ? (
            <Link href={`/courses/c/${nextChapter.id}`}
              className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-3 hover:border-gray-400 hover:bg-gray-50 transition-colors group text-right">
              <p className="text-xs text-gray-400 mb-0.5">Next →</p>
              <p className="text-sm font-medium text-gray-700 group-hover:text-gray-900 truncate">{nextChapter.title}</p>
            </Link>
          ) : (
